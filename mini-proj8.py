...
def rock_spawner():
    ...
    if (len(rock_group) < 12) and (dist(rock_pos, my_ship.get_position()) > 2*(a_rock.get_radius() + my_ship.get_radius()))  and started:
...
def group_group_collide(group_one, group_two):
    collide_num = 0
    for group_one_member in set(group_one):
        if group_collide(group_two, group_one_member):
            collide_num += 1
            group_one.discard(group_one_member)
